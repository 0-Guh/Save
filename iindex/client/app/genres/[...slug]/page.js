...
export function generateMetadata({ searchParams, params: { slug } }) {
    ...
    return { title: `Assistir | ${decodeURIComponent(FormatSlug.join(" | "))} | Page ${Math.max(searchParams.page || 1, 1)}` }
}
...
export default async function Genres({ searchParams, params: { slug } }) {
    ...
    const Response = await fetch("http://192.168.1.16:5000/getDataGenres/" + FormatSlug + "?year=" + getYearParams, { cache: "no-store" })
